...
data_dir = "../data/shirt"
dense = False
save_dir = f"vis/graph-vis-{data_dir.split('/')[-1]}-dense" if dense else f"vis/graph-vis-{data_dir.split('/')[-1]}"
os.makedirs(save_dir, exist_ok=True)

obj_kypts_paths = []
eef_kypts_paths = []
eef_kypts_dir = os.path.join(data_dir, 'dense_eef_kypts' if dense else 'eef_kypts')
eef_kypts_path_list = sorted(glob.glob(os.path.join(eef_kypts_dir, '*.npy')))
obj_kypts_dir = os.path.join(data_dir, 'dense_obj_kypts' if dense else 'obj_kypts')
...
for cam in range(4):
    ...
    intr_list[cam] = np.load(os.path.join(data_dir, f"camera_{cam+1}/camera_intrinsic_params.npy"))
    extr_list[cam] = np.load(os.path.join(data_dir, f"camera_{cam+1}/camera_extrinsic_matrix.npy"))
