...
## RoboCraft
def rotation_matrix_from_quaternion(params):  # params: B x 4
    one = torch.ones(1, 1).to(params.device)
    zero = torch.zeros(1, 1).to(params.device)
...
def get_env_group(args, n_particles, scene_params):
    ...
    p_rigid = p_rigid.to(args.device)
    p_instance = p_instance.to(args.device)
    physics_param = physics_param.to(args.device)
