...
def rollout_rope(args, data_dir, prep_save_dir, save_dir, checkpoint, episode_idx, start_idx, rollout_steps, colormap, n_fps=None):# , dense, rigid_transform=False):
    ...
    physics_dir = os.path.join(prep_save_dir, 'kypts', 'physics')
    physics_path_list = sorted(glob.glob(os.path.join(physics_dir, '*.npy')))

    obj_kypts_path_list = [f for f in obj_kypts_path_list if int(f.split('/')[-1].split('.')[0].split('_')[0]) == episode_idx]
    eef_kypts_path_list = [f for f in eef_kypts_path_list if int(f.split('/')[-1].split('.')[0].split('_')[0]) == episode_idx]
    physics_path_list = [f for f in physics_path_list if int(f.split('/')[-1].split('.')[0].split('_')[0]) == episode_idx]
    if start_idx + rollout_steps + 1 > len(obj_kypts_path_list):
        rollout_steps = len(obj_kypts_path_list) - start_idx - 1

    obj_kypts_paths.extend(obj_kypts_path_list)
    eef_kypts_paths.extend(eef_kypts_path_list)
    physics_paths.extend(physics_path_list)
    ...
    obj_kp = np.load(obj_kp_path).astype(np.float32)   # (2, 8, 3)
   ...
   # physics_param = np.zeros(max_nobj, dtype=np.float32)  # 1-dim
    ...
    physics_path = physics_paths[start_idx]
    ...
    with torch.no_grad():
        for i in range(start_idx + 1, start_idx + 1 + rollout_steps):
            ...
            obj_kp_tmp = np.load(obj_kp_path).astype(np.float32)
            obj_kp_next = [obj_kp_tmp[1]]
            eef_kp = np.load(eef_kp_path).astype(np.float32)   # (2, 8, 3)
            eef_kp_num = eef_kp.shape[1]
...
if __name__ == "__main__":
    ...
    rollout_rope(args, data_dir, prep_save_dir, save_dir, checkpoint, episode_idx, start_idx, rollout_steps, colormap, n_fps)# dense, rigid_transform)
