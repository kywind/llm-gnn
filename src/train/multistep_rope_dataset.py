...
'''
class MultistepRopeDynDataset(Dataset):
    def __init__(self, 
        args,
        data_dirs,
        prep_save_dir,
        ...
        if True:
            ...
            eef_kypts_dir = os.path.join(save_dir, 'kypts', 'eef_kypts')
            eef_kypts_path_list = sorted(glob.glob(os.path.join(eef_kypts_dir, '*.npy')))
            obj_kypts_dir = os.path.join(save_dir, 'kypts', 'obj_kypts')
        ...
        self.max_nobj = 20  # max number of object points
    ...
    def __getitem__(self, i):
        ...
        obj_kp = np.load(obj_kp_path).astype(np.float32)   # (2, N, 3)
        obj_kp_next = [obj_kp[1]]
        obj_kp = [obj_kp[0]]
        eef_kp = np.load(eef_kp_path).astype(np.float32)   # (2, 8, 3)
        eef_kp_num = eef_kp.shape[1]

        # change eef_kp y to mean obj_kp y
        obj_kp_mean_y = np.mean(obj_kp[0][:, 1])
        eef_kp[:, :, 1] = obj_kp_mean_y
        ...
        if n_future > 1:
            ...
            episode_idx = int(obj_kp_path.split('/')[-1].split('.')[0].split('_')[0])
            frame_idx = int(obj_kp_path.split('/')[-1].split('.')[0].split('_')[1])
            # steps = np.load(os.path.join(self.data_dirs[0], f'episode_{episode_idx}', 'steps.npy'))
            ...
            #     if frame_idx >= steps_a[si] + 1 and frame_idx <= steps_a[si+1] - 2:
            #         curr_step = si
            #         break
            # else:
            ...
            for fi in range(n_future - 1):
                ...
                episode_idx_future = int(obj_kp_path_future.split('/')[-1].split('.')[0].split('_')[0])
                frame_idx_future = int(obj_kp_path_future.split('/')[-1].split('.')[0].split('_')[1])
                # print(episode_idx, episode_idx_future, frame_idx, frame_idx_future)
                if episode_idx_future != episode_idx:
                    # exceed the episode
                    obj_kp_futures.extend([obj_kp_next] * (n_future - 1 - fi))
                    obj_future_mask.extend([False] * (n_future - 1 - fi))
                    break
                ...
                obj_kp_future = np.load(obj_kp_path_future).astype(np.float32)   # (2, N, 3)
                # assert obj_kp_future.shape[1] == 100
                obj_kp_future_orig = [obj_kp_future[0]]
                obj_kp_future_orig = [obj_kp_future_orig[j][fps_idx] for j, fps_idx in enumerate(self.fps_idx_list)]
                # if (obj_kp_future_orig - obj_kp_next).sum() > 0.01:
                #     print(obj_kp_future[0].mean(0) - obj_kp_next.mean(0))
                # else:
                #     print('good')
                obj_kp_future = [obj_kp_future[1]]  # NOTE is this correct?
                assert len(obj_kp_future) == 1
                # assert obj_kp_future[0].shape[0] == 100
        ...
        physics_path = self.physics_paths[i]
        physics_param = np.load(physics_path).astype(np.float32)   # (phys_dim)
        physics_param = np.tile(physics_param[None], (max_nobj, 1))  # (N, phys_dim)
        ...
        if n_future > 1:
            ...
            for fi in range(n_future - 1):
                ...
                # cancelling this because we allow moving eef keypoints
