...
class Dataparser:
    ...
    def segment(self, texts):
        ...
        print(prompt)  # things like: [['apple', 'banana', 'orange']]
        ...
        for j in range(boxes.shape[0]):
            ...
            mask = mask * np.array([255, 0, 0])  # red
            mask = Image.fromarray(mask.astype(np.uint8))
            mask_vis = Image.blend(crop_img_pil, mask, 0.7)
            mask_vis.save(f'vis/mask_{j}.png')

            predictions.append((crop_img_pil, mask_vis))
