...
for i in range(len(obj_kypts_paths)):
    ...
    for cam in range(4):
        ...
        for j, kp in [(push_start, obj_kp), (push_end, obj_kp_next)]:
            # print(i, j)
            img_path = os.path.join(data_dir, f'camera_{cam}/color', f'color_{j}.png')
            ...
            cv2.imwrite(os.path.join(save_dir_cam, f'{j:06}.jpg'), img)
